...
const SearchManufacturer = ({ manufacturer, setManufacturer }: SearchManufacturerProps) => {
    ...
    const filteredManufacturers = query === ""
        ...
        : manufacturers.filter((item) => (
            item.korean.includes(query) || item.english.toLowerCase().replace(/\s+/g, "").includes(query.toLowerCase().replace(/\s+/g, ""))
        ));
        ...
        <div className='search-manufacturer'>
            <Combobox value={manufacturer} onChange={setManufacturer}>
                <div className='relative w-full'>
                    ...
                    <Combobox.Input className="search-manufacturer__input" placeholder='기아' displayValue={(manufacturer: { korean: string }) => manufacturer.korean} onChange={(e) => setQuery(e.target.value)} />
                        ...
                        <Combobox.Options>
                            {/* active 되면 스타일링 주기 */}
                            {
                                filteredManufacturers.map((item) => (
                                    <Combobox.Option key={item.korean} className={({ active }) => `relative search-manufacturer__option ${active ? `bg-primary-blue text-white` : 'text-gray-900'}`} value={item}>{({ selected, active }) => (
                                        <>
